// News/index.tsx — Categories from context (no standalone fetch)
...
const News: React.FC = () => {
  ...
  const allCats = ['All', ...newsCategories.map((c) => c.category)];
            ...
            <motion.div layout className={styles.grid}>
              {filtered.map((item, i) => (
                <motion.article
                  ...
                  whileHover={{ y: -4 }}
                >
                  <Link to={`/news/${item.slug}`} onClick={() => trackClick(item.id)} className={styles.cardLink}>
                    ...
                    <div className={styles.body}>
                      <div className={styles.meta}>
                        <span className={styles.metaItem}><Clock size={12} /> {item.readTime} min read</span>
                        <span className={styles.metaItem}>{new Date(item.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                      </div>
